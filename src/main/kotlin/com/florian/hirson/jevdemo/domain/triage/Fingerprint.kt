 ...
 * what lets a cache recognize a recurring error instead of re-classifying
 * it. Wraps a SHA-256 hex digest of the message after [RedactSensitiveData]
...
data class Fingerprint(val hash: String) {
    ...
    companion object {
        ...
        fun of(logEvent: LogEvent): Fingerprint {
            val redacted = RedactSensitiveData.execute(logEvent.message)
            val digest = MessageDigest.getInstance("SHA-256").digest(redacted.toByteArray(Charsets.UTF_8))
